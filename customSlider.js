 ...
 */
 ...
 const setDefaultSettings = (slider, wrapper, activeIndex, slides, buttons, scrollbar) => {
     ...
     setTimeout(() => {
         const width = (slider.getBoundingClientRect().width / 2 - activeSlide.getBoundingClientRect().left) - (activeSlide.getBoundingClientRect().width / 2);
 ...
 export const slider = () => {
     ...
     slider.forEach((slider, i) => {
          ...
          */
         const animationLoopSlide = () => {
             if (BIND_SLIDERS[isBind]) {
                 BIND_SLIDERS[isBind].forEach(item => {
                     checkEndOfSlider(item)
                 });
             } else {
                 checkEndOfSlider(currentSlider);
             }
         };
